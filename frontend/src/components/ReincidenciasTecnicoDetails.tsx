...
const fetchAllDetails = async (empresa: string, tecnico: string, fechaInicio: string, fechaFin: string) => {
    const detailPromise = axios.get<DetalleVisitaData[]>(`${API_URL}/api/reincidencias/detalle-tecnico`, {
        params: { empresa, recurso: tecnico, fecha_inicio: fechaInicio, fecha_fin: fechaFin }
    });
    const evolutionPromise = axios.get<ApiEvolucionData[]>(`${API_URL}/api/reincidencias/evolucion-tecnico`, {
...
//   empresa: string | null; // Acepta null para la carga inicial
//   propietario_red: string;
//   fecha_inicio?: string;
//   fecha_fin?: string;
// }

// export default function ReincidenciasTecnicoDetails({ empresa, propietario_red, fecha_inicio, fecha_fin }: DetailsProps) {
//   const [data, setData] = useState<ReincidenciaData[]>([]);
//   const [summary, setSummary] = useState({ finalizadas: 0, reincidencias: 0, tasaGlobal: 0 });
//   const [loading, setLoading] = useState<boolean>(false); // Inicia en false
//   const [error, setError] = useState<Error | null>(null);
//   const [selectedTechnician, setSelectedTechnician] = useState<string>('');
//   const [detailData, setDetailData] = useState<DetalleVisitaData[]>([]);
//   const [evolutionData, setEvolutionData] = useState<EvolucionData[]>([]);
//   const [detailLoading, setDetailLoading] = useState<boolean>(false);
//   const [detailError, setDetailError] = useState<string | null>(null);
//   const [reincidenceCases, setReincidenceCases] = useState<number>(0);
//   const [activeDateRange, setActiveDateRange] = useState<{ start: string; end: string } | null>(null);
//   const [isTextDialogOpen, setTextDialogOpen] = useState(false);
//   const [textDialogContent, setTextDialogContent] = useState({ title: '', content: '' });
//   const [isDetailModalOpen, setDetailModalOpen] = useState(false);

//   const handleOpenTextDialog = (title: string, content: string) => {
//     setTextDialogContent({ title, content });
...
//     if (!empresa) {
//         setData([]); // Limpiamos datos anteriores si se des-selecciona la empresa
//         setLoading(false);
//         return;
//     }
...
//       setError(null);
//       try {
//         const response = await axios.get<ApiReincidenciaData[]>(`${API_URL}/api/reincidencias/por-tecnico`, {
//           params: { empresa, propietario_red, fecha_inicio: startDate, fecha_fin: endDate }
//         });
//         const fetchedData = response.data;
//         const parsedData: ReincidenciaData[] = fetchedData.map(row => ({
//           recurso: row.recurso,
//           total_finalizadas: parseInt(row.total_finalizadas, 10) || 0,
//           total_reincidencias: parseInt(row.total_reincidencias, 10) || 0,
//           porcentaje_reincidencia: parseFloat(row.porcentaje_reincidencia) || 0,
...
//         const totalReincidencias = parsedData.reduce((sum, row) => sum + row.total_reincidencias, 0);
//         const tasaGlobal = totalFinalizadas > 0 ? (totalReincidencias / totalFinalizadas * 100) : 0;
//         setSummary({ finalizadas: totalFinalizadas, reincidencias: totalReincidencias, tasaGlobal: tasaGlobal });
//       } catch (err) {
//         if (err instanceof Error) setError(err); else setError(new Error('Ocurrió un error desconocido.'));
//       } finally {
//         setLoading(false);
//       }
//     };
...
//   useEffect(() => {
//     if (!isDetailModalOpen || !selectedTechnician || !empresa || !activeDateRange) {
//       return;
//     }
...
//       });
//       const evolutionPromise = axios.get<ApiEvolucionData[]>(`${API_URL}/api/reincidencias/evolucion-tecnico`, {
//         params: { empresa, recurso: selectedTechnician }
//       });
//       try {
//         const [detailResponse, evolutionResponse] = await Promise.all([detailPromise, evolutionPromise]);
//         setDetailData(detailResponse.data);
//         const uniqueServiceCodes = new Set(detailResponse.data.map(row => row['Cod_Servicio']));
//         setReincidenceCases(uniqueServiceCodes.size);
//         const parsedEvolutionData: EvolucionData[] = evolutionResponse.data.map(row => ({
//             ...row,
//             porcentaje_reincidencia: parseFloat(row.porcentaje_reincidencia) || 0
//         }));
//         setEvolutionData(parsedEvolutionData);
//       } catch (err) {
//         if (axios.isAxiosError(err) && err.response) {
//             setDetailError(`Error ${err.response.status}: ${err.response.data.error || 'No se pudo obtener el detalle.'}`);
//         } else if (err instanceof Error) {
//             setDetailError(err.message);
//         } else {
//             setDetailError('Ocurrió un error desconocido.');
//         }
//       } finally {
...
//       return 'reincidence-highlight';
//     }
//     return '';
//   };
  
//   if (!empresa) {
//       return (<Alert severity="info" sx={{ mt: 4 }}>Por favor, selecciona una empresa en la barra superior para comenzar el análisis.</Alert>);
//   }
...
//             {data.length > 0 ? (
//                 <DataGrid
//                     rows={data.map((row, index) => ({ id: row.recurso, posicion: index + 1, ...row }))}
//                         columns={mainColumns}
//                         onRowClick={handleRowClick}
//                         sx={{
...
//         </DialogTitle>
//         <DialogContent dividers>
//           {detailLoading && <CircularProgress sx={{ display: 'block', mx: 'auto', my: 4 }} />}
...
//                   <>
//                     <Alert severity="success" icon="✅" sx={{ mb: 2 }}>
//                         Para <strong>{selectedTechnician}</strong> se han encontrado <strong>{reincidenceCases}</strong> casos de reincidencia en el período seleccionado.
//                     </Alert>
//                     <Typography variant="h6" sx={{ mt: 2, mb: 1 }}>Detalle de Visitas (período seleccionado)</Typography>
//                     <Box sx={{ height: 400, width: '100%' }}>
//                       <DataGrid
...
//                         // CAMBIO FINAL Y DEFINITIVO: Lógica de ID a prueba de fallos.
//                         getRowId={(row) => row['ID externo'] || `${row['Cod_Servicio']}-${row['Fecha Agendamiento']}-${row['Recurso']}`}
...
//                         slotProps={{ toolbar: { showQuickFilter: true } }}
//                         sx={{
//                             '& .reincidence-highlight': { bgcolor: '#FFC7CE', color: '#9C0006', '&:hover': { bgcolor: '#FFB3BD' } },
//                         }}
//                       />
//                     </Box>
//                     <Divider sx={{ my: 4 }} />
...
//                     <Box sx={{ height: 300, width: '100%' }}>
//                       <ResponsiveContainer>
//                         <LineChart data={evolutionData} margin={{ top: 5, right: 40, left: 0, bottom: 5 }}>
//                           <CartesianGrid strokeDasharray="3 3" />
//                           <XAxis dataKey="fecha_periodo" />
//                           <YAxis allowDecimals={false} domain={[0, 'dataMax + 2']} tickFormatter={(tick) => `${tick}%`}/>
//                           <RechartsTooltip formatter={(value) => [`${value}%`, "Tasa Reinc."]} />
