// frontend/src/components/ProduccionProvisionDetails.tsx (REFACTORIZADO)
...
const fetchProduccionProvisionDetails = async (
    empresa: string,
    ...
    return (
        <ResponsiveContainer width="100%" height={500}>
            <ScatterChart margin={{ top: 20, right: 30, bottom: 40, left: 20 }}>
                <CartesianGrid />
                <XAxis type="number" dataKey="total_finalizadas" name="Instalaciones Finalizadas" unit=" inst." />
...
export default function ProduccionProvisionDetails() {
  ...
  const { data: rawData, isLoading, isError, error, isFetching } = useQuery({
    queryKey: ['produccionProvisionDetails', { empresa, propietarioRed, startDate, endDate }],
    queryFn: () => fetchProduccionProvisionDetails(empresa!, propietarioRed, startDate, endDate),
    enabled: !!empresa,
  });
  ...
  const kpiSummaryText = useMemo(() => {
    ...
    if (total === 0) return '';
    const compliant = parsedData.filter(t => t.pct_efectividad >= 80).length;
    const percentage = total > 0 ? ((compliant / total) * 100).toFixed(0) : "0";
    return `En este periodo seleccionado de ${total} técnicos evaluados, el ${percentage}% está cumpliendo con la meta de 80%, equivalente a ${compliant} técnicos.`;
  }, [parsedData]);
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'pct_efectividad', headerName: '% Efectividad', flex: 1, minWidth: 150, type: 'number', align: 'center', headerAlign: 'center',
        ...
        cellClassName: (params) => { const efectividad = params.value ?? 0; return efectividad >= 97 ? 'kpi-positive' : 'kpi-negative'; },
    },
  ];
  ...
  return (
    <Box>
      ...
      <Typography variant="h5" gutterBottom sx={{ mt: 2 }}>Efectividad de Provisión: {empresa}</Typography>
      
      <Box sx={{ display: 'flex', flexWrap: 'nowrap', gap: 2, mb: 3, mt: 2 }}>
        <Card sx={{ flex: '1 1 33%', textAlign: 'center' }}><CardContent><Typography color="text.secondary">Total Asignadas</Typography><Typography variant="h4" sx={{ fontWeight: 'bold' }}>{summary.asignadas.toLocaleString('es-CL')}</Typography></CardContent></Card>
        <Card sx={{ flex: '1 1 33%', textAlign: 'center' }}><CardContent><Typography color="text.secondary">Total Finalizadas</Typography><Typography variant="h4" sx={{ fontWeight: 'bold' }}>{summary.finalizadas.toLocaleString('es-CL')}</Typography></CardContent></Card>
        <Card sx={{ flex: '1 1 33%', textAlign: 'center' }}><CardContent><Typography color="text.secondary">Tasa de Efectividad</Typography><Typography variant="h4" sx={{ fontWeight: 'bold', color: summary.efectividad >= 97 ? '#388E3C' : '#D32F2F' }}>{summary.efectividad.toFixed(2)}%</Typography></CardContent></Card>
      </Box>
...
// // --- Gráfico de Cuadrantes (adaptado para Provisión) ---
// const QuadrantChart = ({ data }: { data: ParsedTecnicoData[] }) => {
//   if (data.length === 0) return null;
//   const avgFinalizadas = data.reduce((sum, item) => sum + item.total_finalizadas, 0) / data.length;
//   const umbralEfectividad = 80; // Meta de 80% de efectividad (equivale a 3% de falla)
...
// export default function ProduccionProvisionDetails({ empresa, propietario_red, fecha_inicio, fecha_fin }: DetailsProps) {
...
//     if (total === 0) return '';
//     const compliant = data.filter(t => t.pct_efectividad >= 80).length;
